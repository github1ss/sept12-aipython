...
def get_pet_labels(image_dir):
    ...
    for file in listdir(image_dir):
        if (file[0] != '.'):
           ...
           pet_label = ""
           file_flds = file.split("_")
           for lbl_part in file_flds:
               lbl_part=lbl_part.strip()
               if not (lbl_part[0].isdigit()):
                   pet_label +=  lbl_part.lower() +  ' '
    ...
    for idx in range(0, len(filenames_list), 1):
        if filenames_list[idx] not in results_dic:
            results_dic[filenames_list[idx]] = [pet_label_list[idx]]
        else:
    ...
    #for k in results_dic:
        #print("KEY={},{}".format(k,results_dic[k]))
        #print(type(results_dic[k]))
